    ...
    <Box sx={{ mb: 3 }}>
      ...
      >
        {sortedColorEntries.map(([fullKey, colorValue]) => {
          ...
          return <ColorSwatch key={fullKey} colorDef={colorValue} label={labelForSwatch} />;
        })}
      </Box>
    </Box>
  )
...
const ColorGuide = () => {
  ...
  const getPaletteColors = (palette: any, prefix: string = '') => {
    ...
    const alertColorDefinitions: Record<string, ColorDef> = {
      ...
      '--mui-palette-Alert-successStandardBg': { type: 'token', value: seagrass[100], displayValue: 'seagrass[100]' },
      '--mui-palette-Alert-successColor': { type: 'token', value: seagrass[600], displayValue: 'seagrass[600]' },
      '--mui-palette-Alert-errorStandardBg': { type: 'token', value: orangered[100], displayValue: 'orangered[100]' },
      '--mui-palette-Alert-errorColor': { type: 'token', value: orangered[700], displayValue: 'orangered[700]' },
      '--mui-palette-Alert-warningStandardBg': { type: 'token', value: persimmon[100], displayValue: 'persimmon[100]' },
      '--mui-palette-Alert-warningColor': { type: 'token', value: persimmon[800], displayValue: 'persimmon[800]' },
      '--mui-palette-Alert-infoStandardBg': { type: 'token', value: bnblue[100], displayValue: 'bnblue[100]' },
      '--mui-palette-Alert-infoColor': { type: 'token', value: bnblue[600], displayValue: 'bnblue[600]' },


      // Filled Variants (from MuiAlert.ts, light mode)
      '--mui-palette-Alert-successFilledBg': { type: 'var', value: 'var(--mui-palette-success-main)' },
      '--mui-palette-Alert-successFilledColor': { type: 'var', value: 'var(--mui-palette-success-contrastText)' },
      '--mui-palette-Alert-errorFilledBg': { type: 'var', value: 'var(--mui-palette-error-main)' },
      '--mui-palette-Alert-errorFilledColor': { type: 'var', value: 'var(--mui-palette-error-contrastText)' },
      '--mui-palette-Alert-warningFilledBg': { type: 'var', value: 'var(--mui-palette-warning-main)' },
      '--mui-palette-Alert-warningFilledColor': { type: 'var', value: 'var(--mui-palette-warning-contrastText)' },
      '--mui-palette-Alert-infoFilledBg': { type: 'var', value: 'var(--mui-palette-info-main)' },
      '--mui-palette-Alert-infoFilledColor': { type: 'var', value: 'var(--mui-palette-info-contrastText)' },
    };
    ...
    Object.entries(palette).forEach(([key, value]) => {
      ...
      if (key === 'action' || key === 'common') {
        Object.entries(value).forEach(([actionKey, actionValue]) => {
          if (!actionKey.includes('Channel')) {
            colors[`${key}.${actionKey}`] = { type: 'var', value: `var(--mui-palette-${key}-${actionKey})` };
          }
        });
        return;
      }
        ...
        Object.entries(value).forEach(([seriesKey, seriesValue]) => {
          if (typeof seriesValue === 'object' && seriesValue !== null) {
            Object.entries(seriesValue).forEach(([shadeKey, shadeValue]) => {
              ...
              if (shadeKey === 'main' && shadeValue && typeof shadeValue === 'string' && !shadeKey.includes('Channel')) {
                colors[`${key}.${seriesKey}.${shadeKey}`] = { type: 'var', value: `var(--mui-palette-${key}-${seriesKey}-${shadeKey})` };
              }
            });
          }
        });
      ...
      if (typeof value === 'object') {
        if ('main' in value || 'light' in value || 'dark' in value || 'contrastText' in value) {
          Object.entries(value).forEach(([shade, shadeValue]) => {
            if (shadeValue && !shade.includes('Channel')) {
              colors[`${key}.${shade}`] = { type: 'var', value: `var(--mui-palette-${key}-${shade})` };
            }
          });
        } else {
          Object.entries(value).forEach(([nestedKey, nestedValue]) => {
            if ((typeof nestedValue === 'string' || typeof nestedValue === 'number') && !nestedKey.includes('Channel')) {
              colors[`${key}.${nestedKey}`] = { type: 'var', value: `var(--mui-palette-${key}-${nestedKey})` };
            }
          });
        }
